...
def render_inventory(config: FoundationConfig) -> str:
    ...
    for key, folder in sorted(config.folders.items()):
        lines.append(f"| `{key}` | {folder.display_name} | `{folder.parent or 'organization root'}` |")
    ...
    for key, network in sorted(config.networks.items()):
        subnets = "<br>".join(
            f"`{subnet.name}` `{subnet.region}` `{subnet.ip_cidr_range}`" for subnet in network.subnets
        ) or "-"
